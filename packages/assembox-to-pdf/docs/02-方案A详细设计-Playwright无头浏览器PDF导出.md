...
```
 业务前端        导出服务(Node)                      渲染集群            外部依赖
┌────────┐  ┌──────────────────────────┐  ┌───────────────────┐  ┌──────────────┐
│ 业务系统 │─▶│ API 层(Fastify)           │─▶│ Chromium 池        │  │ 场景配置服务   │
│ 触发导出 │  │  ├ 任务队列(内存+Redis可换) │  │  ├ 进程 ×N         │  │ 业务 API 集群  │
└────────┘  │  ├ 打印变换(纯函数,可进程内) │  │  └ BrowserContext │  │ 对象存储(OSS)  │
            │  └ 数据预取(可选,D2)        │  │    ×M/进程         │  └──────────────┘
            │ 浏览器池管理器               │  │ 每上下文加载:      │
            │ 指标/日志                    │  │  打印宿主页(静态)  │──▶ 按票证请求业务API
            └────────────┬───────────────┘  └───────────────────┘  └──────────────┘
                         │ PDF 结果 → 对象存储 → 回调/轮询 → 业务前端下载
                         ▼
```
...
└── server/                    # ③ 导出服务：Fastify + Playwright 池
    ├── src/
    │   ├── api/               #    HTTP 契约（§7.1）
    │   ├── queue/             #    任务模型与队列（§7.2）
    │   ├── pool/              #    浏览器池（§7.4）
    │   ├── render/            #    单任务渲染编排（§7.3）
    │   └── metrics/           #    指标采集（§11）
    └── Dockerfile             #    Chromium + Noto CJK（§10）
```

**依赖方向**：`print-transform` ← 仅依赖 `core-next` 类型；`print-host` ← 依赖 `desktop-next` + `print-transform`；`server` ← 仅依赖 Playwright + HTTP 框架，**不依赖** Vue 生态（服务与前端资产解耦，可独立部署扩缩容）。
...
```
GET https://print.internal/print?ticket=eyJhbGciOi...     ← Chromium 加载
宿主页:  ticket → POST https://print.internal/task/claim   ← 换取 { scene, printOptions, dataOverride?, apiCredentials? }
```
...
`printOptions`（首版字段，均为可选）：

```ts
{
  format: 'A4' | 'A3' | 'Letter',        // 默认 A4
  orientation: 'portrait' | 'landscape', // 默认 portrait
  marginsMm: { top, bottom, left, right },        // 默认 12/16/12/12
  header?: { template },                 // HTML 模板，支持 {{title}} {{exportedAt}}
  footer?: { template, showPageNumber: true },
  rowLimit: number,                      // 表格全量上限，默认 1000，防失控
  keepNav: false,
  tolerant: false,                       // 字典类请求失败是否降级
  dataMode: 'browser' | 'prefetch',      // §6.3
  scale: 1.0,                            // Chromium pdf scale
}
```

**同步小文档通道**：`POST /exports?mode=sync`（≤ 3 页、队列空闲时）直接流式返回 PDF，省去轮询；超时 15s 自动降级为异步任务。
...
- **队列**：首版进程内有界队列（`concurrency` 见池参数）+ 基于 Redis 的任务表（崩溃恢复）；预留 BullMQ 接缝，多实例部署时切换；
- **幂等**：`sceneId + printOptions + filters` 指纹去重，10 分钟窗口内直接复用产物；
- **重试**：仅对 `rendering` 阶段的基础设施错误（浏览器崩溃、上下文超时）重试 ≤ 2 次，业务失败（`__ASSEM_PDF_ERROR__`）不重试（重试也不会成功）；
- **TTL**：产物 URL 签名 24h；任务记录保留 7 天。

### 7.3 单任务渲染编排

```ts
async function renderTask(task, pool): Promise<PdfResult> {
  const ticket = await issueTicket(task);            // 一次性票据
  const ctx = await pool.acquireContext();           // §7.4，含排队
  try {
    const page = await ctx.newPage();
    await page.emulateMedia({ media: 'print' });
    const url = `${HOST}/print?ticket=${ticket}`;
    const [loadResp] = await Promise.all([
      page.waitForNavigationOrLoad(url),             // domcontentloaded，不等 networkidle（有轮询请求时会挂死）
      page.waitForFunction('__ASSEM_PDF_ERROR__ in window', null, { timeout: 500 }).catch(() => null), // 尽早失败
    ]);
    if (loadResp && !loadResp.ok()) throw new RenderError('HOST_LOAD_FAILED', String(loadResp.status()));
    await Promise.race([
      page.waitForFunction('__ASSEM_PDF_READY__ === true', null, { timeout: TIMEOUT_RENDER_MS }),   // 默认 30s
      page.waitForFunction('window.__ASSEM_PDF_ERROR__', null, { timeout: TIMEOUT_RENDER_MS })
...
**两级模型：进程（Browser）× 上下文（BrowserContext）。**

| 参数 | 首版默认 | 说明 |
|------|---------|------|
| `browsersPerPod` | 1 | K8s 单容器单进程，OOM 边界清晰 |
| `contextsPerBrowser` | 3–4（可配） | 活跃渲染并发；上下文创建 ~50ms，远轻于进程 |
| `queueMax` | 100 | 超出即 429 快速失败 |
| `taskTimeoutMs` | 30_000 | 硬超时，超时强关上下文 |
| `browserRecycleAfter` | 500 任务 / 30min | 定期轮换，防累积泄漏（Vue app 卸载、G2Plot 实例释放不完全） |

**自愈链**：`page.on('crash')` / `browser.on('disconnected')` → 标记进程不健康 → 池重建 → 排队任务重新派发（受 §7.2 重试上限保护）；连续 3 次重建失败 → Pod 摘除流量（探针失败）。

**暖页优化（P2）**：常驻「预热上下文」预加载宿主页外壳（应用样式、字体、Element Plus），任务到来时同上下文内 `page.goto` 复用 HTTP 缓存，目标把宿主页加载从 ~1.5s 压到 ~400ms。

**启动参数**：`--disable-dev-shm-usage --disable-gpu --no-sandbox --font-render-hinting=none`（`--no-sandbox` 补偿措施见 §9）。
...
| 票据重放/劫持 | ticket 一次性消费 + 60s TTL + 绑定渲染 Pod IP；claim 走内网 |
| 越权数据访问 | 业务凭证 = 发起人上下文短期 token（≤ 5min），单任务内闭环；D2 模式下凭证不出导出服务 |
| `--no-sandbox` 放大利用面 | 容器只读根文件系统 + drop ALL capabilities、独立 nodePool、NetworkPolicy 仅放行配置服务/业务 API/OSS、无敏感 volume |
...
```dockerfile
# server/Dockerfile（骨架）
FROM node:22-slim AS build
# ... pnpm 构建 print-host 静态产物 + server 包 ...

FROM node:22-slim
RUN apt-get update && apt-get install -y --no-install-recommends \
    chromium fonts-noto-cjk libnss3 libatk-bridge2.0-0 libdrm2 libxkbcommon0 \
 && rm -rf /var/lib/apt/lists/*
ENV PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 \
    PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH=/usr/bin/chromium
COPY --from=build /app/server/dist ./dist
COPY --from=build /app/print-host/dist ./static
# 非 root 运行；K8s: requests 1C2G / limits 2C4G，/dev/shm 不依赖（--disable-dev-shm-usage）
CMD ["node", "dist/index.js"]
```

- 用系统 Chromium + 系统 Noto CJK（fontconfig 自动发现），避免 Playwright bundled 浏览器的镜像膨胀；
- K8s：就绪探针 = 池健康 + 队列未满；HPA 按队列深度 + 渲染 P95 双指标扩容；
- 字体更新 = 换基础镜像，随之触发全量视觉回归（§11）。

---

## 11. 可观测性与测试

**指标**（Prometheus）：`export_queue_depth`、`export_task_duration_seconds{phase=queue|render|pdf|upload}`、`export_pdf_pages`、`pool_contexts_active`、`pool_browser_restarts_total`、`export_fail_total{reason}`（区分 HOST_LOAD_FAILED / PAGE_REPORTED / TIMEOUT / CRASH）。

**日志**：每任务一条结构化日志（taskId、sceneId、phase 耗时、页数、字节数、失败原因），`PAGE_REPORTED` 时附带页面回传的首个错误请求名。
...
| P1 MVP | 3 周 | print-transform 库（规则表 + 单测）、导出服务（API+队列+池+超时）、Dockerfile、OSS 归档 | 20 并发压测无泄漏无 OOM；P95 < 10s；失败可回查日志 |
| P2 生产化 | 2–3 周 | 视觉回归流水线、指标告警、暖页优化、D2 预取通道、幂等去重 | SLO：成功率 > 99.5%，P95 < 8s |
...
| `--no-sandbox` 安全评审受阻 | 中 | 备选：Chromium headless=new + seccompProfile；或独立安全域部署 | P1 前 |
...
| Q4 | 导出服务与业务后端的部署关系：独立域 vs 挂业务网关 | 影响凭证换取方式与 NetworkPolicy 范围，需运维参与评审 |
