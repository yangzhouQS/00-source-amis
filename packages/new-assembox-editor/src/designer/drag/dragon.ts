...
export class Dragon {
  ...
  on(callbacks: DragonCallbacks): () => void {
    const added: Array<[keyof DragonCallbacks, (...args: any[]) => any]> = [];
    (Object.keys(callbacks) as Array<keyof DragonCallbacks>).forEach((key) => {
      const fn = callbacks[key] as unknown as (...args: any[]) => any;
